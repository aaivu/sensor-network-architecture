...
public:
    SimpleQNetwork(double lr = 0.001) : learningRate(lr) { initializeWeights(); }
    ...
    void copyFrom(const SimpleQNetwork& other) {
        weights1 = other.weights1; bias1 = other.bias1;
        weights2 = other.weights2; bias2 = other.bias2;
        weights3 = other.weights3; bias3 = other.bias3;
    }
};
...
public:
    PrioritizedReplayBuffer(size_t maxSize, double alpha = 0.6,
                            double beta = 0.4, double epsilon = 1e-6)
        : maxSize(maxSize), currentSize(0), position(0),
    ...
    void add(const Experience& experience) {
        ...
        if (!priorities.empty()) {
            maxPriority = *std::max_element(priorities.begin(),
    ...
    std::vector<Experience> sample(size_t batchSize, std::vector<size_t>& indices,
    ...
    void updatePriorities(const std::vector<size_t>& indices,
                           const std::vector<double>& tdErrors) {
...
public:
     ...
     */
    double calculateAdvancedReward(uint32_t ueId, double pdr, double energyConsumption_mJ,
                                    double sinr, bool packetSuccess, int currentMCS,
                                    double currentTP, int repetitions,
                                    uint32_t packetsSent, bool rachSuccess, int rachAttempts) {
        double reward = 0.0;
        ...
        if (packetSuccess) {
            ...
            if (pdr > 0.9) reward += 0.3;
            else if (pdr > 0.7) reward += 0.1;
        } else {
            reward -= 1.0;
            // Extra penalty for continued failures
            if (pdr < 0.3) reward -= 0.5;
        }
...
struct DeviceHistory {
    ...
    void addRsrp(double rsrp) {
        ...
        if (rsrpHistory.size() > 20) rsrpHistory.pop_front();
    }

    void addSinr(double sinr) {
        sinrHistory.push_back(sinr);
        if (sinrHistory.size() > 20) sinrHistory.pop_front();
    }

    void addPacketResult(bool success, double timestamp) {
        packetResults.push_back(success);
        if (packetResults.size() > 20) packetResults.pop_front();
        timestamps.push_back(timestamp);
        if (timestamps.size() > 20) timestamps.pop_front();
        if (success) {
            consecutiveLosses = 0;
    ...
    void addRachResult(bool success, int attempts) {
        ...
        if (rachResults.size() > 20) rachResults.pop_front();
        if (rachAttemptsHistory.size() > 20) rachAttemptsHistory.pop_front();
        rachAttempts += attempts;
        if (!success) rachFailures++;
    }
    ...
    double getRsrpVariance() const {
        if (rsrpHistory.size() < 2) return 0.0;
        double mean = 0;
        for (auto v : rsrpHistory) mean += v;
        mean /= rsrpHistory.size();
        double variance = 0;
        for (auto v : rsrpHistory) variance += (v - mean) * (v - mean);
        return variance / rsrpHistory.size();
    }

    double getSinrTrend() const {
        if (sinrHistory.size() < 5) return 0.0;
        double recentAvg = 0, oldAvg = 0;
        size_t half = sinrHistory.size() / 2;
        for (size_t i = 0; i < half; i++) oldAvg += sinrHistory[i];
        for (size_t i = half; i < sinrHistory.size(); i++) recentAvg += sinrHistory[i];
        oldAvg /= half;
    ...
    double getPdrTrend() const {
        ...
        for (int i = 0; i < half; i++) if (packetResults[i]) oldSuccess++;
        for (size_t i = half; i < packetResults.size(); i++) if (packetResults[i]) recentSuccess++;
    ...
    double getRachSuccessRate() const {
        if (rachResults.empty()) return 1.0;
    ...
    double getAvgRachAttempts() const {
        if (rachAttemptsHistory.empty()) return 1.0;
        double sum = 0;
        for (auto a : rachAttemptsHistory) sum += a;
...
struct RACHCollisionIndicators {
    ...
    static RACHCollisionIndicators analyze(
        double rsrp,
        double sinr,
        bool packetSuccess,
        bool rachSuccess,
...
public:
    ...
    static ActionResult decodeAction(int action, uint8_t currentMCS, int currentReps,
                                      ResourceUnit currentRU, double currentTP) {
        ActionResult result;
        ...
        switch (repAction) {
            case 0: result.newRepetitions = std::max(1, currentReps / 4); break;
            case 1: result.newRepetitions = std::max(1, currentReps / 2); break;
            case 2: result.newRepetitions = currentReps; break;
            case 3: result.newRepetitions = std::min(128, currentReps * 2); break;
            default: result.newRepetitions = currentReps; break;
        }

        // RU change: narrower, same, wider
        int ruIdx = static_cast<int>(currentRU);
        switch (ruAction) {
            case 0: result.newRU = std::max(0, ruIdx - 1); break;  // Narrower (more robust)
            case 1: result.newRU = ruIdx; break;                     // Same
            case 2: result.newRU = std::min(4, ruIdx + 1); break;  // Wider (more efficient)
            default: result.newRU = ruIdx; break;
        }
...
public:
    DuelingQNetwork(double lr = 0.0005) : learningRate(lr) { initializeWeights(); }
    ...
    void copyFrom(const DuelingQNetwork& other) {
        sharedWeights1 = other.sharedWeights1; sharedBias1 = other.sharedBias1;
        sharedWeights2 = other.sharedWeights2; sharedBias2 = other.sharedBias2;
        valueWeights1 = other.valueWeights1; valueBias1 = other.valueBias1;
        valueWeights2 = other.valueWeights2; valueBias2 = other.valueBias2;
        advWeights1 = other.advWeights1; advBias1 = other.advBias1;
        advWeights2 = other.advWeights2; advBias2 = other.advBias2;
    }
};
...
public:
    ...
    int selectAction(const std::vector<double>& state, uint32_t ueId,
        ...
        if (dist(rng) < epsilon) {
            ...
            auto indicators = RACHCollisionIndicators::analyze(
                history.getLastRsrp(), history.getLastSinr(),
                ...
                history.getRsrpVariance(), history.consecutiveLosses,
                history.getRecentPdr(), cellLoad);
    ...
    double getEpsilon() const { return epsilon; }
    void increaseExploration() { epsilon = std::min(1.0, epsilon * 2.0); }
};
 ...
 */
uint8_t getMandatoryMCSCeiling(double sinr, int repetitions) {
    double effectiveSINR = sinr + GetRepetitionGainDb(repetitions);
    for (int mcs = 10; mcs >= 0; mcs--) {
        if (effectiveSINR >= MCS_TABLE[mcs].requiredSINR_dB + 2.0) { // 2 dB margin
            return mcs;
        }
    }
    return 0;
    ...
    double energyConsumed_mJ)
{
    ...
    if (packetSuccess) {
        ...
        if (recentPdr >= 0.9) reward += outcomeWeight * 0.3;
    } else {
        reward -= outcomeWeight * 1.0;
        if (consecutiveLosses > 5) reward -= outcomeWeight * 0.5;
    }
...
private:
    ...
    double tanh_fn(double x) { return std::tanh(x); }
    double relu(double x) { return x > 0 ? x : 0; }
...
public:
    ...
    std::pair<std::vector<double>, std::vector<double>> forward(const std::vector<double>& input) {
        ...
        for (int i = 0; i < HIDDEN_SIZE; i++) {
            double sum = bias1[i];
            for (int j = 0; j < INPUT_SIZE; j++) sum += weights1[i][j] * input[j];
            h1[i] = tanh_fn(sum);
        }
        ...
        for (int i = 0; i < HIDDEN_SIZE; i++) {
            double sum = bias2[i];
            for (int j = 0; j < HIDDEN_SIZE; j++) sum += weights2[i][j] * h1[j];
            h2[i] = tanh_fn(sum);
        }
        ...
        for (int i = 0; i < OUTPUT_SIZE; i++) {
            ...
            for (int j = 0; j < HIDDEN_SIZE; j++) sum += weightsMean[i][j] * h2[j];
            mean[i] = sum;
        }
        ...
        for (int i = 0; i < OUTPUT_SIZE; i++) {
            ...
            for (int j = 0; j < HIDDEN_SIZE; j++) sum += weightsLogStd[i][j] * h2[j];
...
public:
    ValueNetwork(double lr = 0.001) : learningRate(lr) { initializeWeights(); }
...
public:
    PPOAgent() : rng(std::random_device{}()) {}
    ...
    void train() {
        if (trajectoryBuffer.empty()) return;
        ...
        for (int t = trajectoryBuffer.size() - 1; t >= 0; t--) {
            double nextValue = (t == (int)trajectoryBuffer.size() - 1) ?
                               lastValue : trajectoryBuffer[t + 1].value;
...
public:
     ...
     */
    std::vector<double> buildMARLState(
        const DeviceHistory& history,
        uint8_t currentMCS,
        int currentReps,
        double currentTP,
    ...
    int selectAction(const std::vector<double>& state, uint32_t ueId,
        ...
        if (dist(rng) < epsilon) {
            ...
            if (indicators.likelyCongestion) {
                ...
                for (auto& [id, act] : otherAgentActions) actionCounts[act]++;
 ...
 */
void SetupInterCellInterference(double cellRadius, uint32_t nNeighborCells) {
    std::cout << "  Setting up " << nNeighborCells << " neighboring cells for interference..." << std::endl;
    ...
    for (uint32_t i = 0; i < nNeighborCells; i++) {
        ...
        std::cout << "    Neighbor cell " << i << " at (" << pos.x << ", " << pos.y << ")" << std::endl;
    }
 ...
 */
void SetupLTEDataInterferers(double cellRadius, uint32_t nInterferers) {
    lteDataInterferers.Create(nInterferers);

    MobilityHelper mobility;
    mobility.SetMobilityModel("ns3::RandomWalk2dMobilityModel",
        "Bounds", RectangleValue(Rectangle(-cellRadius, cellRadius, -cellRadius, cellRadius)),
    ...
    std::cout << "  Deployed " << nInterferers << " mobile LTE data interferers (human mobility)" << std::endl;
}
...
void InitializeHardwareVariability(uint32_t nUEs, NodeContainer& ueNodes) {
...
RsrpSample SamplePreTxRsrp(uint32_t ueNodeId, Time currentTime) {
    ...
    if (enableEnvironmentalModeling && ueHardwareVariance.find(ueNodeId) != ueHardwareVariance.end()) {
 ...
 */
double CalculateNbiotEnergy(uint8_t mcs, int repetitions, double txPower_dBm,
                             ResourceUnit ru, int rachAttempts,
                             uint32_t payloadBytes, double psmSleepTime_s,
                             CELevel ceLevel) {
    if (globalEnergyModel) {
        return globalEnergyModel->CalculateTransmissionCycleEnergy(
            txPower_dBm, mcs, repetitions, ru, rachAttempts,
            payloadBytes, psmSleepTime_s, ceLevel);
    }
 ...
 */
void UpdateDefaultLinkAdaptation(uint32_t ueId, double sinr, bool packetSuccess,
                                   double rsrp, bool rachSuccess, int rachAttempts) {
    ...
    if (sinrHist.size() > 20) sinrHist.pop_front();

    // Average SINR over last 20 measurements
    double avgSINR = 0;
    for (auto s : sinrHist) avgSINR += s;
    ...
    // Select MCS with 3 dB margin for reliability
    uint8_t newMCS = 0;
    for (int m = 10; m >= 0; m--) {
        if (effectiveSINR >= MCS_TABLE[m].requiredSINR_dB + 3.0) {
            newMCS = m;
            break;
        }
    }
    ...
    NS_LOG_INFO("Default LA - UE " << ueId << ": MCS=" << (int)newMCS
                << " Reps=" << reps << " TP=" << newTP << " dBm"
 ...
 */
void UpdateDDQNADR(uint32_t ueId, double sinr, bool packetSuccess,
                    double rsrp, bool rachSuccess, int rachAttempts) {
    ...
    auto state = agent->buildRealisticState(
        history, currentMCS[ueId], currentRepetitions[ueId],
        currentTP[ueId], currentRU[ueId], cellLoad, batteryLevel,
        Simulator::Now().GetSeconds());

    // Estimate congestion
    double congestion = estimateCellCongestion(
        rsrp, history.getRsrpVariance(), history.getRecentPdr(),
        ueMetrics.size(), history.getAvgRachAttempts(), cellLoad);

    // Calculate energy for this transmission
    double energy = CalculateNbiotEnergy(
        currentMCS[ueId], currentRepetitions[ueId], currentTP[ueId],
        static_cast<ResourceUnit>(currentRU[ueId]), rachAttempts,
        20, globalAppPeriodSeconds, CELevel::CE0);

    // Calculate reward
    double reward = calculateCongestionAdaptiveReward(
        ueId, packetSuccess, rachSuccess, rachAttempts,
        rsrp, sinr, history.getRsrpVariance(),
        history.consecutiveLosses, currentMCS[ueId], currentTP[ueId],
        currentRepetitions[ueId], currentRU[ueId],
        history.getRecentPdr(), history.getPdrTrend(),
        Simulator::Now().GetSeconds() - history.lastSuccessTime,
        congestion, energy);

    // Select action
    int action = agent->selectAction(state, ueId, history,
                                      Simulator::Now().GetSeconds(), cellLoad);

    // Decode action to NB-IoT parameters
    auto result = ExtendedActionSpace::decodeAction(
        action, currentMCS[ueId], currentRepetitions[ueId],
        static_cast<ResourceUnit>(currentRU[ueId]), currentTP[ueId]);
    ...
    if (previousStates.find(ueId) != previousStates.end()) {
        agent->addExperience(previousStates[ueId], previousActions[ueId],
                             reward, state, false);
        agent->train(32);
    }
    ...
    NS_LOG_INFO("DDQN LA - UE " << ueId
                << ": MCS=" << (int)currentMCS[ueId]
                << " Reps=" << currentRepetitions[ueId]
                << " RU=" << (int)currentRU[ueId]
                << " TP=" << currentTP[ueId]
                << " ε=" << std::fixed << std::setprecision(3) << agent->getEpsilon()
                << " R=" << std::setprecision(2) << reward);
}

/**
 * PPO Link Adaptation for NB-IoT
 */
void UpdatePPOADR(uint32_t ueId, double sinr, bool packetSuccess,
                   double rsrp, bool rachSuccess, int rachAttempts) {
    auto& history = deviceHistories[ueId];
    auto& metrics = ueMetrics[ueId];
    ...
    std::vector<double> actionVec = {
        static_cast<double>(action.mcs),
        std::log2(action.repetitions),
        action.txPower
    };
    ...
    if (prevStates.find(ueId) != prevStates.end()) {
        ppoAgent->addTransition(prevStates[ueId], prevActions[ueId],
                                 reward, state, false);
    }
 ...
 */
void UpdateMARLADR(uint32_t ueId, double sinr, bool packetSuccess,
                    double rsrp, bool rachSuccess, int rachAttempts) {
    auto& history = deviceHistories[ueId];
    auto& metrics = ueMetrics[ueId];
    ...
    auto state = agent->buildMARLState(
        history, currentMCS[ueId], currentRepetitions[ueId],
        currentTP[ueId], currentRU[ueId], cellLoad, batteryLevel,
        Simulator::Now().GetSeconds(), numActive, avgCellMCS, avgCellReps);

    // Update coordination signals from other agents
    for (auto& [otherId, otherAgent] : marlAgents) {
        if (otherId != ueId) {
            agent->updateCoordination(otherId, 0, currentMCS[otherId],
                                       currentRepetitions[otherId]);
        }
    }
    ...
    double congestion = estimateCellCongestion(
        rsrp, history.getRsrpVariance(), history.getRecentPdr(),
        numActive, history.getAvgRachAttempts(), cellLoad);
    ...
    if (prevStates.find(ueId) != prevStates.end()) {
        agent->addExperience(prevStates[ueId], prevActions[ueId],
                             reward, state, false);
        agent->train(32);
    }
 ...
 */
void SimulateUplinkTransmission(uint32_t ueIdx) {
    ...
    if (rachSuccess) {
        ...
        if (globalEnb) {
            auto uplinkResult = globalEnb->ProcessUplink(
                ueId, rxPower, mcs, reps, ru, Simulator::Now());
            ...
            double interCellInterf = globalChannel->CalculateInterCellInterference(
                nodePositions[ueId], Simulator::Now());
    ...
    double energy = CalculateNbiotEnergy(mcs, reps, tp, ru, rachAttempts,
                                          20, globalAppPeriodSeconds, CELevel::CE0);
    ...
    ueMetrics[ueId].lastPDR = ueMetrics[ueId].packetsSent > 0 ?
        static_cast<double>(ueMetrics[ueId].packetsReceived) / ueMetrics[ueId].packetsSent : 0.0;
    ...
    Simulator::Schedule(Seconds(globalAppPeriodSeconds),
                        &SimulateUplinkTransmission, ueIdx);
}
...
void WriteCSVOutput(const std::string& filename) {
    ...
    for (const auto& record : completedPackets) {
        csv << std::fixed << std::setprecision(3)
            ...
            << record.rsrq_dB << ","
            << record.sinr_dB << ","
            << record.preTxRssi_dBm << ","
            << record.rachSuccess << ","
            << record.rachAttempts << ","
            << record.dataSuccess << ","
            << record.airtime_ms << ","
            << record.energyConsumed_mJ << ","
            << (int)record.ceLevel << ","
            << record.couplingLoss_dB << ","
            << record.payloadBytes << ","
            << record.cellLoad << ","
            << record.collisionFlag
            << std::endl;
    }

    csv.close();
    std::cout << "CSV output written: " << filename << " (" << completedPackets.size() << " records)" << std::endl;
}

/**
 * Write environment visualization CSV
 */
void WriteEnvironmentVisualization(const std::string& baseFilename, double radius, uint32_t nUEs) {
    ...
    for (auto& [id, pos] : nodePositions) {
        if (id == 0) continue; // Skip eNB
        double distance = std::sqrt(pos.x * pos.x + pos.y * pos.y);
        csv << "UE," << id << "," << pos.x << "," << pos.y << "," << pos.z
            << ",dist=" << std::fixed << std::setprecision(0) << distance << "m" << std::endl;
    }
    ...
    for (int i = 0; i < 6; i++) {
        ...
        csv << "neighbor_eNB," << i << ","
            << isd * std::cos(angle) << "," << isd * std::sin(angle) << ",30,"
            << "interferer" << std::endl;
    }
 ...
 */
uint8_t getInitialMCSForDistance(double distance) {
    if (distance < 200) return 8;   // Close: high MCS
    if (distance < 500) return 5;   // Medium: mid MCS
    if (distance < 1000) return 2;  // Far: low MCS
 ...
 */
int getInitialRepetitionsForDistance(double distance) {
    if (distance < 200) return 1;
    if (distance < 500) return 2;
    if (distance < 1000) return 8;
    if (distance < 2000) return 32;
    return 64;
}
...
double getInitialTPForDistance(double distance) {
    if (distance < 100) return 14.0;
    if (distance < 500) return 20.0;
    return 23.0;
}
...
// Forward declaration
void RunSimulation(uint32_t nUEs, double simulationTime, double appPeriodSeconds,
                   double cellRadius, const std::string& csvFileName,
                   ADRMethod adrMethod, uint32_t nLteInterferers, uint32_t nNeighborCells);
...
int main(int argc, char* argv[]) {
    ...
    cmd.AddValue("adr", "Link adaptation mode: 'off', 'on', 'ddqn', 'ppo', 'marl', or 'all'", adrModeStr);
    ...
    cmd.AddValue("neighborCells", "Number of neighbor cells for inter-cell interference", nNeighborCells);
    cmd.Parse(argc, argv);
    ...
    if (enableEnvironmentalModeling) {
        std::cout << "  - Inter-cell interference (" << nNeighborCells << " neighbor cells)" << std::endl;
        std::cout << "  - In-band LTE traffic (" << nLteInterferers << " mobile devices)" << std::endl;
    ...
    if (adrModeStr == "all") {
        ...
        RunSimulation(nUEs, simulationTime, appPeriodSeconds, cellRadius,
                      "nbiot_datasets/no_la_" + csvFileName, ADRMethod::OFF, nLteInterferers, nNeighborCells);

        std::cout << "\n[2/5] Running with DEFAULT link adaptation..." << std::endl;
        RunSimulation(nUEs, simulationTime, appPeriodSeconds, cellRadius,
                      "nbiot_datasets/default_la_" + csvFileName, ADRMethod::ON, nLteInterferers, nNeighborCells);

        std::cout << "\n[3/5] Running with DDQN-PER link adaptation..." << std::endl;
        RunSimulation(nUEs, simulationTime, appPeriodSeconds, cellRadius,
                      "nbiot_datasets/ddqn_la_" + csvFileName, ADRMethod::DDQN, nLteInterferers, nNeighborCells);

        std::cout << "\n[4/5] Running with PPO link adaptation..." << std::endl;
        RunSimulation(nUEs, simulationTime, appPeriodSeconds, cellRadius,
                      "nbiot_datasets/ppo_la_" + csvFileName, ADRMethod::PPO, nLteInterferers, nNeighborCells);

        std::cout << "\n[5/5] Running with MARL link adaptation..." << std::endl;
        RunSimulation(nUEs, simulationTime, appPeriodSeconds, cellRadius,
                      "nbiot_datasets/marl_la_" + csvFileName, ADRMethod::MARL, nLteInterferers, nNeighborCells);
        ...
        if (adrModeStr == "on") {
            adrMethod = ADRMethod::ON; filePrefix = "default_la_";
        } else if (adrModeStr == "ddqn") {
            adrMethod = ADRMethod::DDQN; filePrefix = "ddqn_la_";
        } else if (adrModeStr == "ppo") {
            adrMethod = ADRMethod::PPO; filePrefix = "ppo_la_";
        } else if (adrModeStr == "marl") {
            adrMethod = ADRMethod::MARL; filePrefix = "marl_la_";
        } else {
            adrMethod = ADRMethod::OFF; filePrefix = "no_la_";
        }
        RunSimulation(nUEs, simulationTime, appPeriodSeconds, cellRadius,
                      "nbiot_datasets/" + filePrefix + csvFileName, adrMethod, nLteInterferers, nNeighborCells);
    }
...
void RunSimulation(uint32_t nUEs, double simulationTime, double appPeriodSeconds,
    ...
    if (adrMethod == ADRMethod::ON) modeLabel = "DEFAULT (3GPP)";
    else if (adrMethod == ADRMethod::DDQN) modeLabel = "DDQN-PER";
    else if (adrMethod == ADRMethod::PPO) modeLabel = "PPO";
    else if (adrMethod == ADRMethod::MARL) modeLabel = "MARL";
    ...
    if (adrMethod == ADRMethod::DDQN || adrMethod == ADRMethod::PPO || adrMethod == ADRMethod::MARL) {
        std::string agentType = (adrMethod == ADRMethod::DDQN ? "DDQN-PER" :
        ...
        for (uint32_t i = 0; i < nUEs; i++) {
            ...
            if (adrMethod == ADRMethod::DDQN) {
                ...
                ddqnAgents[nodeId] = std::make_unique<DDQNPERADRAgent>(
                    0.01, 0.5, 0.998, 0.1, 0.9, 50, 1.0, 0.2);
            ...
            std::cout << "  UE " << nodeId << ": dist=" << std::fixed << std::setprecision(0)
                      ...
                      << ", Reps=" << currentRepetitions[nodeId]
                      << ", TP=" << currentTP[nodeId] << "dBm" << std::endl;
        }

        if (adrMethod == ADRMethod::DDQN) {
            std::cout << "  ✅ Using Dueling DQN with extended action space (60 actions)" << std::endl;
    ...
    for (uint32_t i = 0; i < nUEs; i++) {
        ...
        std::cout << "  UE " << ueNodes.Get(i)->GetId()
                  << ": first TX at " << std::fixed << std::setprecision(1)
                  << startTime << "s, period=" << appPeriodSeconds << "s" << std::endl;
    }
    ...
    for (const auto& record : completedPackets) {
        totalSent++;
        if (record.dataSuccess && record.rachSuccess) totalReceived++;
    ...
    std::cout << "Avg Energy per Packet: " << std::setprecision(2) << avgEnergy << " mJ" << std::endl;
