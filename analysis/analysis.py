...
ADR_NAMES = {
    ...
    'ppo_adr': 'PPO ADR',
    'marl_adr': 'MARL ADR',
    'mappo_gat_adr': 'MAPPO-GAT ADR',
    'marl_ppo_adr': 'MARL-PPO ADR'
}

ADR_ORDER = ['no_adr', 'classical_adr', 'ddqn_adr', 'ppo_adr', 'marl_adr', 'mappo_gat_adr', 'marl_ppo_adr']
...
class LoRaWANADRAnalyzer:
    ...
    def load_datasets(self):
        ...
        patterns = {
            ...
            'mappo_gat_adr': 'mappo_gat_adr_advanced_pre_tx_rssi_dataset_device_*_dataset.csv',
            'marl_ppo_adr': 'marl_ppo_adr_advanced_pre_tx_rssi_dataset_device_*_dataset.csv'
        }
    ...
    def generate_comprehensive_analysis(self):
        ...
        for device_id in all_devices:
            ...
            for adr_type in ['no_adr', 'classical_adr', 'ddqn_adr', 'ppo_adr', 'marl_adr', 'hybrid_adr', 'mappo_gat_adr', 'marl_ppo_adr']:
    ...
    def create_performance_visualizations(self, all_stats, device_comparison):
        ...
        adr_mapping = {
            ...
            'ppo_adr': 'PPO',
            'marl_adr': 'MARL',
            'mappo_gat_adr': 'MAPPO-GAT',
            'marl_ppo_adr': 'MARL-PPO'
        }
        adr_order = ['no_adr', 'classical_adr', 'ddqn_adr', 'ppo_adr', 'marl_adr', 'mappo_gat_adr', 'marl_ppo_adr']
...
# ---------------------------------------------------------------------------
# Standalone chart helpers — work directly from analyzer.device_data so they
# succeed even when generate_comprehensive_analysis() returns empty.
# ---------------------------------------------------------------------------
...
def _device_pdrs(analyzer, adr_type):
    """Return list of per-device PDR values (0–100) for one ADR type."""
    pdrs = []
    for df in analyzer.device_data.get(adr_type, {}).values():
        if len(df) == 0:
            continue
        if 'packet_received' in df.columns:
            pdrs.append(df['packet_received'].sum() / len(df) * 100)
        else:
            pdrs.append((df['snr_db'] != -999.0).sum() / len(df) * 100)
    return pdrs


def _device_energies(analyzer, adr_type):
    """Return list of per-device total energy (mJ) for one ADR type."""
    energies = []
    for df in analyzer.device_data.get(adr_type, {}).values():
        if 'energy_consumed_mJ' in df.columns and len(df) > 0:
            energies.append(df['energy_consumed_mJ'].sum())
    return energies


def _bar_chart(ax, labels, means, stds, ylabel, title, unit=''):
    """Draw an annotated bar chart on ax."""
    bars = ax.bar(labels, means, yerr=stds,
                  color=_BAR_FILL, edgecolor=_BAR_EDGE, linewidth=0.8,
                  capsize=4, error_kw={'linewidth': 0.8})
    ylim = max((m + s for m, s in zip(means, stds)), default=1.0) * 1.20
    ax.set_ylim(0, ylim)
    for bar, mean, std in zip(bars, means, stds):
        top = bar.get_height() + std
        lbl = f'{mean:.1f}{unit}' if mean < 1000 else f'{mean/1000:.1f}k{unit}'
        ly = top + ylim * 0.015
        va = 'bottom'
        if ly + ylim * 0.04 > ylim:
            ly, va = bar.get_height() - ylim * 0.02, 'top'
        ax.text(bar.get_x() + bar.get_width() / 2, ly,
                lbl, ha='center', va=va, fontsize=9, fontweight='bold')
...
def _plot_pdr_from_device_data(analyzer):
    """Generate pdr_by_adr_type.png from raw device data."""
    present = [t for t in ADR_ORDER if analyzer.device_data.get(t)]
    if not present:
        return
    labels = [_SHORT_NAMES.get(t, t) for t in present]
    means = [np.mean(_device_pdrs(analyzer, t)) if _device_pdrs(analyzer, t) else 0 for t in present]
    stds  = [np.std(_device_pdrs(analyzer, t))  if _device_pdrs(analyzer, t) else 0 for t in present]
    plt.rcParams.update(_CHART_STYLE)
    fig, ax = plt.subplots(figsize=(10, 4.8))
    _bar_chart(ax, labels, means, stds, 'Packet Delivery Rate (%)',
               'Packet Delivery Rate (PDR) by ADR Type', unit='%')
    ax.set_ylim(0, 115)
    fig.tight_layout()
    out = analyzer.output_dir / 'pdr_by_adr_type.png'
    fig.savefig(out, format='png', dpi=300, bbox_inches='tight')
    print(f"PDR chart: {out}")
    plt.close(fig)


def _plot_energy_from_device_data(analyzer):
    """Generate energy_by_adr_type.png from raw device data."""
    present = [t for t in ADR_ORDER if analyzer.device_data.get(t)]
    if not present:
        return
    labels = [_SHORT_NAMES.get(t, t) for t in present]
    means = [np.mean(_device_energies(analyzer, t)) if _device_energies(analyzer, t) else 0 for t in present]
    stds  = [np.std(_device_energies(analyzer, t))  if _device_energies(analyzer, t) else 0 for t in present]
    plt.rcParams.update(_CHART_STYLE)
    fig, ax = plt.subplots(figsize=(10, 4.8))
    _bar_chart(ax, labels, means, stds,
               'Total Energy Consumed (mJ)', 'Energy Consumption by ADR Type')
    fig.tight_layout()
    out = analyzer.output_dir / 'energy_by_adr_type.png'
    fig.savefig(out, format='png', dpi=300, bbox_inches='tight')
    print(f"Energy chart: {out}")
    plt.close(fig)


def _plot_fairness_from_device_data(analyzer):
    """Compute and plot Jain's Fairness Index from loaded device data."""
    present_types = [t for t in ADR_ORDER if t in analyzer.device_data]
    ...
    for adr_type in present_types:
        pdrs = []
        for device_id, df in analyzer.device_data[adr_type].items():
            total = len(df)
            if total > 0:
                success = df['packet_received'].sum() if 'packet_received' in df.columns \
                          else (df['snr_db'] != -999.0).sum()
                pdrs.append(success / total)
        fairness_values.append(compute_jains_fairness(np.array(pdrs)) if pdrs else 0.0)
    ...
    labels = [SHORT_NAMES.get(t, ADR_NAMES.get(t, t)) for t in present_types]

    plt.rcParams.update({'font.family': 'serif', 'font.size': 11})
    fig, ax = plt.subplots(figsize=(10, 4.8))
    BAR_FILL, BAR_EDGE = '#A8C8E8', '#2B5D8A'

    bars = ax.bar(labels, fairness_values, color=BAR_FILL, edgecolor=BAR_EDGE, linewidth=0.8)
    ax.set_ylim(0, 1.15)
    ax.axhline(y=1.0, color='#2ecc71', linestyle='--', linewidth=1.0, alpha=0.8,
    ...
    ax.set_xlabel('ADR Method')
    ax.set_title('PDR Fairness by ADR Type')
    ax.tick_params(axis='x', rotation=45, labelsize=8)
    plt.setp(ax.get_xticklabels(), ha='right', rotation_mode='anchor')
    ...
    out_path = analyzer.output_dir / 'fairness_by_adr_type.png'
...
def main():
    ...
    # Always generate the three GUI charts directly from device data,
    # so they appear even if generate_comprehensive_analysis() had no data.
    for fn, name in [(_plot_pdr_from_device_data, 'PDR'),
                     (_plot_energy_from_device_data, 'Energy'),
                     (_plot_fairness_from_device_data, 'Fairness')]:
        try:
            fn(analyzer)
        except Exception as e:
            print(f"{name} chart skipped: {e}")

    print(f"GRAPHS_DIR:{analyzer.output_dir.resolve()}")
